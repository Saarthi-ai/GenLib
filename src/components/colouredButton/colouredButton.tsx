...
const ColouredButton: React.FC<ProgressProps> = ({
  actionType,
  recommended,
  colour,
  text,
  ...
  return (
    <div>
      <div onClick={() => handleUploadClick()} >
        <div role="button" className={style.buttonContainer} style={{ ['--colour' as string]: colour }}>
          <div className={style.buttonContent}>{text} <img src={icon} alt="dsfad" /></div> {recommended && <sup>{starSvg}</sup>}
        </div>
        {recommended && <p className={style.recommendLabel} style={{ ['--colour' as string]: colour }}>recommended</p>}
      </div>
      {actionType === 'upload' && (
        <input
          type="file"
